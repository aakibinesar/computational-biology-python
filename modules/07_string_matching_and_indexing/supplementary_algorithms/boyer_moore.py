...
def bad_character_table(S):
    ...
    """
    if len(S) == 0:
        return [[] for a in range(len(S))]

    R = [[-1] for a in range(len(S))]

    alpha = [-1 for a in range(len(S))]
...
def good_suffix_table(S):
    ...
    N = z_algorithm(S[::-1]) # S[::-1] reverses S
...
def full_shift_table(S):
    ...
    suffix of S. In the cases it is used, the shift magnitude of the pattern P relative to the
...
def string_search(P, T):
    ...
    k = len(P)     # Represents alignment of end of P relative to T
